...
```{r}
#| cache: true
fit_1 <- brm(formula = death_age ~ treatment + party + win_margin,
             data = x,
             family = gaussian(),
             refresh = 0,
             silent = 2,
...
fit_1 |>  
  ...
  ggplot(aes(x = .epred, y = fct_reorder(party, win_margin), fill = party)) + 
